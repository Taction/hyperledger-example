...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_COUCH up -d 2>&1
  else
...
function execcli {
  docker exec cli scripts/org1.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  # todo 完善脚本的输入数据
  docker exec cli scripts/org3.sh 2
  docker exec cli scripts/org3.sh 3
  docker exec cli scripts/org3.sh 4
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! exec cli failed"
    exit 1
  fi
}
...
COMPOSE_FILE=docker-compose-order.yaml
...
if [ "${MODE}" == "up" ]; then
  ...
  networkDown
elif [ "${MODE}" == "execcli" ]; then ## exec the network
  execcli
